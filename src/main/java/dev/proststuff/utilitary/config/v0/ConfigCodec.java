...
public record ConfigCodec<T>(@Nullable Codec<T> codec, @Nullable StreamCodec<RegistryFriendlyByteBuf, T> streamCodec) {
    ...
    public @NonNull Codec<T> getCodec() throws UnsupportedOperationException {
        if (codec == null) throw new UnsupportedOperationException("No defined codec is present");
        return codec;
    }

    public @NonNull StreamCodec<RegistryFriendlyByteBuf, T> getStreamCodec() throws UnsupportedOperationException {
        if (streamCodec == null) throw new UnsupportedOperationException("No defined stream codec is present");
        return streamCodec;
    }
}