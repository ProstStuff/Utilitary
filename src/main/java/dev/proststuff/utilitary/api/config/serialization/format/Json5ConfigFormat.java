...
import dev.proststuff.utilitary.api.config.ConfigConstants;
...
public class Json5ConfigFormat implements ConfigFormat<Json5ConfigFormat.FormatSettings> {
    ...
    public <C, M extends ConfigMetadata> @NonNull FormatSettings create(SimpleIdentifier fileName, ConfigType<C, M, FormatSettings> type) {
        return new FormatSettings(JSON5, type.metadata().comment(), Comment.EMPTY);
    }
    ...
    public static FormatSettings withComment(ConfigType<?, ?, ?> type, Comment comments) {
        return new FormatSettings(JSON5, type.metadata().comment(), comments);
    }
